...
class Calendar extends CI_Controller {
	...
	{
		parent::__construct();
		$this->table 		= 'tb_transaksi';
		$this->load->model('Globalmodel', 'modeldb'); 
	}

	public function detail($idvilla) 
	{
		$data_calendar = $this->modeldb->get_list($idvilla);
		$calendar = array();
		foreach ($data_calendar as $key => $val) 
		{
			$calendar[] = array(
				'id' 	=> intval($val->kode_transaksi), 
				'title' => $val->nama_penyewa, 
				'start' => date_format( date_create($val->tgl_start) ,"Y-m-d H:i:s"),
				'end' 	=> date_format( date_create($val->tgl_end) ,"Y-m-d H:i:s"),
				'color' => '#000',
			);
		}
		...
		$data2['get_villa'] = $this->modeldb->get_villa($idvilla);
		$this->load->view('calendar', $data + $data2);
	}
